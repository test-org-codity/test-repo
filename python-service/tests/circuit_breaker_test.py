...
class CircuitBreakerOpenError(Exception):
    def __init__(self, name: str, remaining_time: float) -> None:
        ...
        msg = (
            ...
            f"Retry after {self.remaining_time:.1f}s."
        )
        super().__init__(msg)
...
class CircuitBreaker:
    ...
    def get_or_create(
        ...
        breaker = CircuitBreaker(name, config=config)
    ...
    def execute(self, operation: Callable[..., Any], *args, fallback=None, **kwargs):
    ...
    def _record_failure(self, duration: float) -> None:
        ...
        if self._state == CircuitState.CLOSED:
            self._failure_count += 1
            # open if count exceeds threshold
            if self._failure_count >= self.config.failure_threshold:
                self._transition_to(CircuitState.OPEN)
                return
            # or if failure rate in full window exceeds threshold
    ...
    def _calculate_failure_rate(self) -> float:
        ...
        return failures / float(len(self._sliding_window)) if self._sliding_window else 0.0
...
class DistributedCircuitBreakerCoordinator:
    def __init__(self, coordinator_url: str, sync_interval: float = 5.0):
        ...
        self.node_id = os.getenv("NODE_ID") or f"python-{os.getpid()}"
...
def circuit_breaker(name: str, config: Optional[CircuitBreakerConfig] = None):
    def decorator(func: Callable):
        breaker = CircuitBreaker.get_or_create(name, config=config)
