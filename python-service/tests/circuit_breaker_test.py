...
def test_circuit_breaker_state_transitions_open_sets_opened_at_and_increments_metrics(breaker):
    ...
    # Some implementations auto-transition OPEN->HALF_OPEN on next state access
    # if timeout is interpreted as elapsed (or 0). Validate OPEN internal markers.
...
def test_circuit_breaker_record_failure_half_open_transitions_to_open(breaker):
    ...
    # Some implementations may re-evaluate and set HALF_OPEN only after timeout; ensure it opened internally.
    assert breaker._state == CircuitState.OPEN
    assert breaker._opened_at == pytest.approx(100.0)
