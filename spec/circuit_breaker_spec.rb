...
RSpec.describe Net::HTTP do
  describe '.new' do
    ...
    it 'defaults port to nil when host is nil (matches Net::HTTP behavior)' do
      http = described_class.new(nil)
      expect(http.address).to be_nil
      expect(http.port).to be_nil
    end
  end
  ...
  describe '#port' do
    ...
    it 'returns nil port if initialized with nil host and no port' do
      http = described_class.new(nil)
      expect(http.port).to be_nil
    end
  end
